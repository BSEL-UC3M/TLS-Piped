#ifndef _itkTreeSegmentationImageFilter_txx
#define _itkTreeSegmentationImageFilter_txx
...
void TreeSegmentationImageFilter<TInputImage, TOutputImage>
...
template <typename TInputImage, typename TOutputImage>
TreeSegmentationImageFilter<TInputImage, TOutputImage>
...
segmentImage, PotentialImagePointer potentialImage, float timeStep)
{
  ...
  float growth=0;
  InputPixelType threshold = m_fix_threshold;;
...
segmentImage, PotentialImagePointer potentialImage, float timeStep)
{
  ...
  if (trialMinHeap.size() == 0) {
   //std::cout << "WARNING! trialMinHeap.size() == 0 returning original segment and stopping propagation" << std::endl;
   return inputSegment;
  }

/*  if ( pTopTrial == NULL) {
    std::cout << "WARNING! trialMinHeap.top() == NULL returning original segment" << std::endl;
    return inputSegment;
    }
    */
  float init_time = trialMinHeap.top().Potential;
  ...
  typename SegmentSpatialObject::BlobPointType newPoint;
  float growth=0;
  InputPixelType threshold = m_var_threshold;
...
::ComputeViscosity(InputImageConstPointer inputImage, InputIndexType* index, InputPixelType* threshold)
{
  ...
  radius.Fill(1);
  NeighborhoodIteratorType neighIt( radius, inputImage, inputImage->GetLargestPossibleRegion() );
  neighIt.SetLocation(*index);
  
  InputPixelType imageValue = 0;
  int countForMean = 0;
  
  // compute imageValue as mean in 26 neighborhood
  for(int t=0; t < neighIt.Size() ; t++)
    {
    //std::cout << neighIt.GetPixel(t);
    imageValue = imageValue + neighIt.GetPixel(t);
    countForMean++;
    }
  imageValue = imageValue/countForMean;
  
  // return viscosity depending on mean value in neighborhood
  float viscosity = INFINITY;
...
::EvaluatePropagation(SegmentPointerType segment, OutputImagePointer outputImage, OutputImagePointer segmentImage)
{
 ...
 // check radius by getting minimum radius of parents
  std::vector<SegmentSpatialObject::Pointer> segments;
  segments.push_back(segment);
  //std::cout << "Airway number: "<<m_AirwayNumber << std::endl;
  int ID = 0;
    
  // iterator through accepted segments
  typename std::vector<SegmentSpatialObject::Pointer>::iterator it, end;
  it = m_AcceptedSegments.begin();
  end = m_AcceptedSegments.end();
    
  float minRadius = 1000;
  
  /*while(!segments.empty())
    {
    if(segments.front()->HasParent())
      {
      ID = segments.front()->GetParent()->GetId();
      it = m_AcceptedSegments.begin();
      std::cout << "In segments loop" << ID<< std::endl;
      // look for segments with that ID
      while(it!=end)
        {
        if(ID==(*it)->GetId())
          {
          if((*it)->GetMinRadius()<minRadius)
            {
            minRadius = (*it)->GetMinRadius();
            }
  ...
  while(!segments.empty())
    ...
    if(segments.front()->HasParent())
      {
      SegmentSpatialObject * auxSegment = dynamic_cast<
           SegmentSpatialObject *> (segments.front()->GetParent());
      auxRadius = (auxSegment->GetMinRadius()+auxSegment->GetMeanRadius())/2;
      if(auxRadius < minRadius)
        {
        minRadius = auxRadius;
        }
       segments.push_back(auxSegment);
  ...
  //std::cout << "Min radius is: " << minRadius << std::endl;
  float radius = sqrt(segment->GetWavefront().size()/3.1416);
    
  OutputIteratorType outIt = OutputIteratorType(outputImage, outputImage->GetLargestPossibleRegion());
  typename SegmentSpatialObject::PointType p;
  typename OutputImageType::IndexType index;
  typename SegmentSpatialObject::PointListType wavefront = segment->GetWavefront();
  typename SegmentSpatialObject::PointListType::iterator wavefrontIt, wavefrontEnd;
  
  // the radius is compared to a min radius depending of the current radius
  // if it is small, the tolerance is lower
  float BetaRadius = 0;
  if (radius < 10)
    {
    BetaRadius = m_BetaRadius;
    }
  else
    {
    BetaRadius = m_BetaRadius + 1;
    }
  if (radius > BetaRadius*minRadius)
    {
    //remove invalid propagation from output image and from segment
    
    wavefrontIt = wavefront.begin();
    wavefrontEnd = wavefront.end();
    
    while(wavefrontIt!=wavefrontEnd)
      {
      p = (*wavefrontIt).GetPosition();
      index[0] = p[0];
      index[1] = p[1];
      index[2] = p[2];
      outIt.SetIndex(index);
    ...
    segment->RemoveLastAddedPoints(wavefront.size());
    return false;
    }
    
  /// check how many collisions with other segments occur
  
  // define iterator
  typename NeighborhoodOutputIteratorType::RadiusType neighborhood;
  neighborhood.Fill(1);
  NeighborhoodOutputIteratorType imIt = NeighborhoodOutputIteratorType(neighborhood, segmentImage, segmentImage->GetLargestPossibleRegion());
  
  // get neighbors already calculated
  std::vector<int> neighbors;
  typename std::vector<int>::iterator nIt, nEnd;
  
  
  // iterate through wavefront voxels checking for new collisions
  wavefrontIt = wavefront.begin();
  wavefrontEnd = wavefront.end();
  typename OutputImageType::PixelType val =0;
  ...
  while(it!=end)
    ...
    index[1] = p[1];
    index[2] = p[2];
    imIt.SetLocation(index);
    for(int k=0; k < imIt.Size() ; k++)
      {
      val = imIt.GetPixel(k);
      neighbors = segment->GetNeighborSegments();
      nIt = neighbors.begin();
      nEnd = neighbors.end();
      if(val!=0)
        // check that it not already counted
        {
        alreadyCounted = false;
        while(nIt!=nEnd && !alreadyCounted)
          {
          if(val==*nIt)
            {
            alreadyCounted = true;
      ...
      if(!alreadyCounted&&val!=0)
        {
        segment->AddNeighborSegment(val);
        //std::cout << val << std::endl;
        }
      }
    ++it;
    }
  // if there are more than two neighbors, we do not accept propagation
  //std::cout << segment->GetNeighborSegments().size() << std::endl;
  
  
    
  if(segment->GetNeighborSegments().size()>2)
    {
    wavefrontIt = wavefront.begin();
    wavefrontEnd = wavefront.end();
...
::EvaluateSegmentAndPossiblyAddToTree(SegmentPointerType segment, OutputImagePointer outputImage, OutputImagePointer segmentImage)
{
  ...
  // we check growth rate
  //std::cout << "Growth rate: " << segment->GetGrowthRate() << std::endl;
  
  // we compute compactness
  float compactness = this->ComputeCompactness(segment, outputImage);
  //std::cout << "Compactness: " << compactness << std::endl;
  
  // rules for accepting/rejecting
  float growthRate = segment->GetGrowthRate();
  
  
  bool accept = true;
  
  if( (growthRate > m_MaxGrowthRate && m_AirwayNumber>2) || (growthRate>1.05 && segment->GetPoints().size()>2500 && segment->GetPoints().size()<10000 && m_AirwayNumber>2)) // for the first airway we allow in pricniple arbitrary growing
    {
    accept = false;
    }
   ...
   {
   if(compactness < 0.60 && m_AirwayNumber>30)
     {
     accept = false;
     }
   }
...
::GenerateData()
{
  ...
  std::cout << "(X) Gamma:" << m_Gamma <<  std::endl;
  std::cout << "Sigma [propagation]:" << m_PropagationSigma <<  std::endl;
  std::cout << "(X) Sigma [sobel filter]:" << m_SobelSigma <<  std::endl;
  std::cout << "Beta radius:" << m_BetaRadius <<  std::endl;

  std::cout << "(X) Lower threshold:" << m_Threshold_lr <<  std::endl;
  std::cout << "Maximum growth rate:" << m_MaxGrowthRate <<  std::endl;
  std::cout << "(X) Gradient threshold:" << m_GradientThreshold <<  std::endl;
  std::cout << "Time step:" << m_TimeStep <<  std::endl;
    
  std::cout << "Variable threshold propagation initial value:" << m_var_threshold <<  std::endl;
  std::cout << "Fixed threshold propagation value:" << m_fix_threshold <<  std::endl;
