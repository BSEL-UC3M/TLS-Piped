...
int main(int argc, char * argv[]) {
	...
	if ((strcmp(argv[1], "--help") == 0) || ((strcmp(argv[1], "-h") == 0))) {
		std::cout << "Usage: " << argv[0]
				<< " InputObjectsMapFile OutFileName [label]" << std::endl;
		return EXIT_SUCCESS;
	}
	...
	if (m_label == 0) {
		...
		std::cout << "Calculating image center: REMEMBER!!!!!!!!!!!" << std::endl;
		std::cout << "Calculated by pixel Spacing cause object centroid are (or use to be): COULD INTRODUCE ERRORS!!!" << std::endl;
		...
		float cx = inputPtr->GetSpacing()[0] * inputPtr->GetLargestPossibleRegion().GetSize()[0] / 2.0;
		float cy = inputPtr->GetSpacing()[1] * inputPtr->GetLargestPossibleRegion().GetSize()[1] / 2.0;
		float cz = inputPtr->GetSpacing()[2] * inputPtr->GetLargestPossibleRegion().GetSize()[2] / 2.0;
		...
		for (cand = candids.begin(); cand != candids.end(); ++cand) {
			...
			int y_obj_start = (*cand)->GetBoundingBox().GetIndex()[1] * inputPtr->GetSpacing()[1];

			if (y_obj_start > cy) {
				std::cout<< " The y start point for this object is very low...skipping because it can be the bed..."<<y_obj_start  <<std::endl;
				continue;
			}
	...
	if (minL.IsNotNull()) {
		...
		std::cout << "    Lungs should be " << minL->GetLabel() << " "
				<< minL->GetNumberOfPixels() << " " << minL->GetCentroid()
				<< std::endl;
		std::cout
				<< "    Outputting original and binarized version of the lungs "
				<< std::endl;
		...
		//masker->SetOrigin(inputPtr->GetOrigin());

		masker->Update();

		std::cout << " Final image orientation is "
				<< masker->GetOutput()->GetDirection() << std::endl;
